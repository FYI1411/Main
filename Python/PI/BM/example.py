...
def send_packet(packetData, packetEnum):
    ...
    s = struct.Struct(
        'h' + str(packet_size) + 's')  # create a data structure with an int and a string (with the string size)
...
# shortcut for server commands
def say(msg, color="16750940"): 
    send_packet('rawsay "' + msg + '" "' + color + '"', rcon_receive.command.value)

def command(cmd, args=[]):
    input = cmd + " "
    for arg in args:
        input += '"' + arg + '" '
    send_packet(input, rcon_receive.command.value) 
...
if connected == 1:  # use 'connected' to start the main loop if a connection is made
    while not keyboard.is_pressed('cmd'):
        ...
        try:
            ...
            # set it to something lower if you're having network performance problems
        ...
        while True:  # a second while loop is needed to loop through the buffer to make sure all data is processed
            if buffer.find(end_read) != -1 and buffer.find(
                    start_read) != -1:  # this if-else statement checks if the data buffer has the beginning delimiter and the end delimiter
                start_index = buffer.find(start_read)  # find the beginning byte position of the Boring Man packet
                end_index = buffer.find(
                    end_read) + 3  # find the ending byte position of the Boring Man packet and offset it to make sure its included (1 character string = 3 bytes)
                data = buffer[
                       start_index:end_index]  # carve out the complete packet out of the data buffer and assign it to the 'data' variable for processing
                buffer = buffer[
                ...
                if data != b'':  # make sure the data variable isn't empty
                    data_info = struct.unpack_from('<' + '3s' + 'h', data,
                                                   0)  # read the beginning delimiter character, and then the JSON string size (in bytes)
                    event_data = struct.unpack_from('<' + '3s' + 'h' + 'h' + str(data_info[1]) + 's', data,
                                                    0)  # read the beginning delimiter character, and then the JSON string size, then the event ID integer, then the JSON string itself using the size value from the line above
                    ...
                    # print("EVENT ID: "+str(event_id)+" - JSON: "+str(message_string)) if event_id in [33] else 0
                    #
                    ...
                    if event_id == rcon_event.player_death.value:
                        ...
                        js = json.loads(message_string)
                        try:
                            victim_id = int(js['VictimProfile'].split(',')[0].split(': ')[-1][1:-1])
                        except ValueError:
                            break
                        source = requests.get('https://steamid.io/lookup/76561198158275076').text
                        soup = bs.BeautifulSoup(source, 'lxml')
                        soup = soup.find('dl', class_="panel-body dl-horizontal")
                        results = {key.string: value.string for key, value in zip(soup.find_all('dt', class_="key"),
                                                                                  soup.find_all('dd', class_="value"))}
                        username = target.get(results['name'], {})
                        if username:
                            say(username + ' died, what a loser' + '!')
                    #
                    # a work around for commands
                    if event_id == rcon_event.log_message.value:
                        js = json.loads(
                            message_string)  # message_string is the json string, so pass it through the json module
                        timestamp = str(datetime.datetime.now().replace(tzinfo=datetime.timezone.utc).strftime("%H:%M:%S"))  # generate a time stamp
                        try:
                            input = js['Message'].split(': ', 1)
                            username = input[0]  # get the username of the player who sent the message
                            chat = input[1]  # get the message text that was sent
                            # print("Received chat message from player " + username)  # notify
                            if chat.startswith('!time'):  # check if the message starts with the command '!time'
                                say('Hello ' + str(username) + '! The time is ' + str(timestamp) + ' UTC.') # reply back to the server with an in-game console command
                            if username == 'Pergon':
                                if chat.startswith('!pause'):
                                    command('pause')
                                elif chat.startswith('!lobby'):
                                    command('lobby')
                                elif chat.startswith('!kick'):
                                    command('kick', chat.split('/')[1:])
                                elif chat.startswith('!ban'):
                                    command('ban', chat.split('/')[1:])
                                elif chat.startswith('!addbot'):
                                    command('addbot', chat.split('/')[1:])
                        except IndexError: # ignore if message doesn't have 'username:'
                            pass
                    # this doesn't work for me spas cause 36 (rcon_event.chat_message is not recieved)
                    ...
                    if event_id == rcon_event.chat_message.value:
                        ...
                        if id != -1:  # ignore the message if it was sent by the server (ID will be -1)
                            ...
                            if chat.startswith('!time'):  # check if the message starts with the command '!time'
                                send_packet('rawsay "Hello ' + str(username) + '! The time is ' + str(timestamp) + '."',
                                            rcon_receive.command.value)  # reply back to the server with an in-game console command
            else:
